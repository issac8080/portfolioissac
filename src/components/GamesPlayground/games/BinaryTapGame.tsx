...
export default function BinaryTapGame({ onScore }: { onScore?: (score: number) => void }) {
  ...
  useEffect(() => {
    ...
    setWrong(false);
    setShowPattern(true);
    const t = setTimeout(() => setShowPattern(false), 1500 + level * 200);
  ...
  }, [input, pattern, level, score, onScore, showPattern]);
    ...
    <div className="flex flex-col items-center gap-6 w-full max-w-sm">
      ...
      {!showPattern && !wrong && (
        <div className="flex gap-4">
          <button type="button" onClick={() => tap(0)} className="min-w-[64px] min-h-[64px] w-16 h-16 rounded-lg bg-ai-surface border border-ai-border text-2xl font-mono text-white touch-manipulation active:scale-95 transition-transform">0</button>
          <button type="button" onClick={() => tap(1)} className="min-w-[64px] min-h-[64px] w-16 h-16 rounded-lg bg-ai-surface border border-ai-border text-2xl font-mono text-white touch-manipulation active:scale-95 transition-transform">1</button>
        </div>
      )}
